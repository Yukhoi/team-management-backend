...
import com.yukai.team.teamservice.service.impl.OutboxEventServiceImpl;
...
class OutboxEventServiceImplTest {
    ...
    @Test
    void should_include_current_user_operator_in_event_payload() throws Exception {
        ...
        outboxEventService.saveEvent("player", 10L, "player.updated", Map.of("playerId", 10L));

        JsonNode payload = objectMapper().readTree(capturingEntityManager.persistedOutboxEvent.getPayloadJson());
        assertEquals(1L, payload.get("operator").get("userId").asLong());
        assertEquals("admin", payload.get("operator").get("username").asText());
        assertEquals(10L, payload.get("data").get("playerId").asLong());
    }

    @Test
    void should_include_system_operator_when_current_user_is_missing() throws Exception {
        CapturingEntityManager capturingEntityManager = new CapturingEntityManager();
        OutboxEventServiceImpl outboxEventService = outboxEventService(capturingEntityManager);

        outboxEventService.saveEvent("team", 20L, "team.created", Map.of("teamId", 20L));
