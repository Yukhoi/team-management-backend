...
public class MatchGoalCreatedHandler implements StatisticsEventHandler {
    ...
    public StatisticsEventType supports() {
        return StatisticsEventType.MATCH_GOAL_CREATED;
    }

    @Override
    public void handle(StatisticsEventEnvelope event) {
        MatchGoalCreatedEventData eventData = objectMapper.convertValue(event.getData(), MatchGoalCreatedEventData.class);
        playerStatsProjectionService.applyGoalCreated(eventData);
        leaderboardProjectionService.rebuildLeaderboards(eventData.getSeasonSnapshot(), eventData.getTournamentId());
        log.info("match.goal.created processed, eventId={}, goalId={}", event.getEventId(), eventData.getGoalId());
    }
}
